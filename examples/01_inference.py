...
def step2_trajectory_inference(model):
    ...
    images = [np.zeros((256, 256, 3), dtype=np.uint8) for _ in range(NUM_STEPS + WINDOW_SIZE - 1)]
    ...
    for step in range(NUM_STEPS):
        input_images = np.stack(images[step:step + WINDOW_SIZE])[np.newaxis]  # (1, W, H, W, C)
        observation = {
